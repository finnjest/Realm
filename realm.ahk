...
Gui Add, Radio, x528 y308 w88 h23 vAlph, Alphabetically
Gui Add, Radio, x624 y308 w135 h23 vAlphRev, Alphabetically (Reverse)
Gui Add, Radio, x528 y332 w90 h23 vLineLength, Line Length
Gui Add, Radio, x624 y332 w135 h23 vLineLengthRev, Line Length (Reverse)
Gui Add, Radio, x528 y380 w93 h23 vFlip, Upside Down
Gui Add, Radio, x528 y356 w90 h23 vNatural, Natural
Gui Add, Radio, x624 y356 w120 h23 vNaturalRev, Natural (Reverse)
...
DellActionButton:
    ...
    Case "Delete Block":
        Result := DeleteBlock(inputText, DeletionEditField1, DeletionEditField2, isLineContext, removeSymbol, caseInsensitive, wholeWordsOnly)
    }
...
DeleteBlock(inputText, DeletionEditField1, DeletionEditField2, isLineContext, removeSymbol, caseInsensitive, wholeWordsOnly) {
    ; Set regex options based on case sensitivity
    regexOptions := CaseInsensitive ? "i)" : ""
...
SortStrings:
    ...
    else if (AlphRev) ; Обратная сортировка по алфавиту
    {
        Sort, InputText, R
    }
    ...
    else if (LineLength) ; По длине строки
    {
        Sort, InputText, F SortFunc
    }
    else if (LineLengthRev) ; Обратная сортировка по длине строки
    {
        Sort, InputText, F RevSortFunc
    }
    else if (Natural) ; Естественная сортировка
    {
        Sort, InputText, F NaturalSortFunc
    }
    else if (NaturalRev) ; Естественная сортировка (обратная)
    {
